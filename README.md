...
```bash
php artisan vendor:publish --tag="laravel-openapi-cli-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="laravel-openapi-cli-views"
```

## Usage

```php
$openApiCli = new Spatie\OpenApiCli();
echo $openApiCli->echoPhrase('Hello, Spatie!');
```
